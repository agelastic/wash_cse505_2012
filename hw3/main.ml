...
let rec interpret heap env e =
  ...
  | Fun(v,t,e1) -> 
    (* ( print_string ("\n function: var " ^ v ^ " tobody " ^ exp2string e1 ^"\n"); *)
    heap, Closure(e,env)
  (* ) *)
  | Apply(e1,e2) -> 
    (* (print_string ("\n application: " ^ exp2string e1 ^ " to " ^ exp2string e2 ^"\n"); *)
    let (h',v1) = interpret heap env e1 in
    let (h'',v2) = interpret h' env e2 in
    (match v1 with
  ...
  | Get(e1) -> 
    (match interpret heap env e1 with
       (h',Label(l)) -> 
       (* (print_string ("\n got value of: " ^ l ^ " is " ^ exp2string (lookup h' l) ^"\n"); *)
       h', lookup h' l
     (* ) *)
     | _ -> raise RunTimeError)
  | Set(e1,e2) ->
    (match interpret heap env e1 with
       ...
       (* (print_string ("\n setting: " ^ l ^ " to " ^ exp2string e2 ^"which is " ^ exp2string v1 ^"\n"); *)
       update h'' l v1, v1
     (* ) *)
     | _ -> raise RunTimeError)
  | Alloc(e1) ->
